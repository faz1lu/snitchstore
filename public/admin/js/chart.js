...
$.ajax({
  ...
  method:'POST',
  success:((res)=>{
   console.log(res.data,'pie chart');
      var doughnut = document.getElementById("doChart");
      if (doughnut !== null) {
        var myDoughnutChart = new Chart(doughnut, {
          type: "doughnut",
          data: {
            labels: ["Deliverd", "Cancelled", "pending", "Confim"],
            datasets: [
              {
                label: ["Deliverd", "Cancelled", "pending", "Confimed"],
                data: res.data,
                backgroundColor: ["#50d7ab", "#dc3545", "#9586cd", "#f3d676"],
                borderWidth: 1
  ...
  $.ajax({
   ...
   success:((res)=>{
    if(res.status){
      ...
      console.log(res.ordersTotal,res.salesprofit,res.orderCount);
     var acquisition = document.getElementById("acquisition");
     if (acquisition !== null) {
      var acqData = [
   
        {
          third: res.orderCount,
